...
- **Flydigi:**
  - **Vader 5 Pro** — **Supported** over **wired USB** and the Flydigi **2.4 GHz USB dongle** (physically confirmed). Works via the existing XInput / DInput host paths depending on pad mode; no dedicated driver. **Bluetooth** and **NS (Switch) mode** are **not supported** — use USB or the dongle in a supported PC / XInput / DInput mode.
